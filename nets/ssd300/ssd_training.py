...
class MultiboxLoss(object):
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        partitions = tf.reduce_sum(tf.one_hot(full_indices, tf.shape(tf.reshape(conf_loss, [-1]))[0], dtype='int32'), 0)

        # neg_conf_loss = tf.gather(tf.reshape(conf_loss, [-1]), full_indices)
        neg_conf_loss = tf.dynamic_partition(tf.reshape(conf_loss, [-1]), partitions, 2)  # (batch_size, n_dim)
        neg_conf_loss = neg_conf_loss[1]
