...
def weighted_loss(y_pred, y_true):
    def init_x(shape=(3, 3), dtype=None):
        return K.constant(np.array([[-1, 0, 1], [-1, 0, 1], [-1, 0, 1]]).reshape((3, 3, 1, 1)))

    def init_y(shape=(3, 3), dtype=None):
        return K.constant(np.array([[1, 1, 1], [0, 0, 0], [-1, -1, -1]]).reshape((3, 3, 1, 1)))

    test = y_true
    grad_x = K.conv2d(test, init_x(), strides=(1, 1), padding="same")
    grad_y = K.conv2d(test, init_y(), strides=(1, 1), padding="same")
    #raise Exception(grad_x.shape)

    mse = K.mean(K.square(y_pred - y_true), axis=-1)
    grad_x_error = K.mean(K.square(y_pred - y_true) * grad_x)
    grad_y_error = K.mean(K.square(y_pred - y_true) * grad_y)
    # wx = K.variable(value=80)
    # wy = K.variable(value=80)
    return K.mean(mse + (grad_x_error * 50 + grad_y_error * 50 )/2)
